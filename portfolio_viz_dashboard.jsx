...
export default function App() {
  ...
  const [topsisN, setTopsisN] = useState(20); // how many assets to display
  ...
  const topsisResults = useMemo(() => {
    ...
    const norms = TCRITERIA.map(c =>
      Math.sqrt(RAW.reduce((s,d) => s + d[c.key] ** 2, 0)) || 1
    );
    ...
    const idealBest  = TCRITERIA.map((c,j) =>
      c.benefit ? Math.max(...weighted.map(d => d.wv[j])) : Math.min(...weighted.map(d => d.wv[j]))
    );
    const idealWorst = TCRITERIA.map((c,j) =>
      c.benefit ? Math.min(...weighted.map(d => d.wv[j])) : Math.max(...weighted.map(d => d.wv[j]))
    );
    ...
    return weighted.map(d => {
      ...
      return { ...d, dPlus, dMinus, cScore, wv: d.wv };
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      setChatMessages(prev => [...prev, { role:"assistant", content:"⚠️ API error — please try again." }]);
    }
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"320px 1fr", gap:16 }}>
              ...
              <div style={{ background:"rgba(255,255,255,0.03)", border:"1px solid #1E293B", borderRadius:12, padding:20 }}>
                ...
                {TCRITERIA.map((c,i) => {
                  ...
                  return (
                    <div key={c.wKey} style={{ marginBottom:14 }}>
                      ...
                      <input
                        type="range" min={0} max={100} value={tWeights[c.wKey]}
                ...
                {/* Reset */}
                <button
                  onClick={() => setTWeights({ Return:30, Risk:25, Liquidity:15, ESG:15, Dividend:10, Beta:5 })}
                ...
                <table style={{ width:"100%", borderCollapse:"collapse", fontSize:11 }}>
                  <thead>
                    <tr style={{ borderBottom:"1px solid #1E293B" }}>
                      <th style={{ textAlign:"left",  padding:"6px 10px", color:"#475569", fontWeight:600 }}>#</th>
                      <th style={{ textAlign:"left",  padding:"6px 10px", color:"#475569", fontWeight:600 }}>Asset</th>
                      <th style={{ textAlign:"left",  padding:"6px 10px", color:"#475569", fontWeight:600 }}>Sector</th>
                      {TCRITERIA.map((c,i) => (
                        <th key={c.key} style={{ textAlign:"right", padding:"6px 10px", color: CCOLORS[i], fontWeight:600 }}>
                          {c.label}<br/>
                          <span style={{ fontSize:9, color:"#334155", fontWeight:400 }}>{c.benefit?"▲":"▼"} {Math.round((tWeights[c.wKey]/totalW)*100)}%</span>
                        </th>
                      ))}
                      <th style={{ textAlign:"right", padding:"6px 10px", color:"#00D4FF", fontWeight:600 }}>D+</th>
                      <th style={{ textAlign:"right", padding:"6px 10px", color:"#6EE7B7", fontWeight:600 }}>D−</th>
                      <th style={{ textAlign:"right", padding:"6px 10px", color:"#FCD34D", fontWeight:700 }}>C-score</th>
                    </tr>
                  </thead>
                  <tbody>
                    {topN.map((d,i) => (
